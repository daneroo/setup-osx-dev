// Config section
/*jshint node: true, indent: 2, strict: false,  es5: true, undef:true */
/* Hint explanations
  undef:true   // This option prohibits the use of explicitly undeclared variables
  strict: false // disable: This option requires all functions to run in EcmaScript 5's strict mode
  es5: true     // required to allow reserved words as express.static
*/
...
var reflectIncoming = [];
// temporary to debug posts
var persistentFeeds = {};
//by accountId - > array of scopes [0,1,2,3,4] : Live,...
...
function persistFeed(accountId, feeds) {
  ...
  function empty() {
    var A = Array;
    return new A(maxScopeId + 1);
  }
  // setup the slots
  persistentFeeds[accountId] = persistentFeeds[accountId] || empty();
  ...
  function track(feeds) {
    ...
    try {
      ...
      names = _.map(feeds, function (feed) {
        return feed.name;
      }).join(',');
    } catch (e) {}
...
['spec.sample.json', 'spec.sampleBy2.json'].forEach(function (sampleDataFileName) {
  var path = require('path').join(__dirname, sampleDataFileName);
  var content = require('fs').readFileSync(path, 'utf8');
  var sample = JSON.parse(content);
  // console.log(sample);
  persistFeed(sample.accountId, sample.feeds);
  // make sure the samples has all scopes
});
...
var jsonrpc_services = require('connect-jsonrpc')(services);
...
dnode(function (client, conn) {
  ...
  conn.on('end', function () {
    ...
    if (idx !== (-1)) {
      clientsByType[client.type].splice(idx, 1);
    }
